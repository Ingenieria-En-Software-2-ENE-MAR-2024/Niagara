...
const TableFilter: React.FC<TableFilterProps> = ({ columns, rows, setFilteredRows }) => {
    ...
    const handleFilter = () => {
        const tempKey = filterColumn === 'ID Paciente' ? "id" : filterColumn === 'Nombre y Apellido' ? "fullName" : filterColumn === 'Área o Especialidad' ? "specialty" : ""
        if (filterColumn === "") {
    ...
    return (
        <div style={{ marginTop: '0px', marginBottom: '20px', justifyContent: 'flex-end', display: 'flex', alignItems: 'center' }}>
            <TextField
                select
                label="Filtrar por"
                value={filterColumn}
