...
type RoutesInfo = {[key : Role] : {roles : Role[], test : (roles : Role[], role : Role | null) => boolean}}
...
function fulfills_any_role(roles : Role[], role : Role | null) : boolean {
  return (role !== null) && (role !== undefined) && (roles.filter( x => has_role(role,x)).length != 0);
}
...
export function check_privileges(url : URL, user_role : Role | null) : "loading" | "authorized" | "unauthorized" {


  let authorized = false;
  routesInfoKeys.forEach( (pattern : string) => {
    authorized = authorized || (url.pathname.startsWith(pattern) &&  test_route(pattern,user_role))
  } ) 
