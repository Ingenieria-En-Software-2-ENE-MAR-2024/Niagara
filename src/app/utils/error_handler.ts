...
export class custom_error extends Error {
  ...
  constructor(error_message: string, error_message_detail: string,  error_code: string,status: number,) {
    super(error_message);
...
export const handle_error_http_response = (error:any,error_code: string) : error_object =>{
    var error_object
    if (error instanceof z.ZodError){
      error_object = {error_message:get_message_for_code(error_code), error_message_detail:'Error in validation',error_code,status:400}
     
    }else  {
      console.log(error.message)
      error_object = {error_message:get_message_for_code(error_code),error_message_detail:error.message, error_code,status:500}
